...
/**
 * Paket soal latihan pribadi untuk mode offline.
 * Menyertakan kunci jawaban — hanya untuk bank soal milik user (bukan tugas kelas).
 */
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const limit = Math.min(Number(req.nextUrl.searchParams.get("limit") ?? 40) || 40, 60);
    ...
    return NextResponse.json(
      { success: false, error: { code: "INTERNAL_ERROR", message: "Gagal menyiapkan paket offline." } },
      { status: 500 }
    );
  }
}
